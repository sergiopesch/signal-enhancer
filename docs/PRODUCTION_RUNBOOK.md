...
paid infrastructure, credentials, and physical-device evidence that cannot be committed or
...
Before any live deployment, record an owner and budget for each service:

- a Vercel Pro or Enterprise project in `dub1`; the committed five-minute cleanup schedule is rejected
  by Hobby and live data retention cannot use the demo's former daily cadence;
- Neon Postgres and a dedicated private Vercel Blob store in the intended production environment;
...
Creating the endpoint and changing the Vercel plan can incur charges. They are explicit release
decisions, not automated repository steps.
...
| `DATABASE_URL`                                   | Production Neon connection, scoped through the integration                  |
...
| `MAX_GLOBAL_JOBS_PER_DAY`, `MAX_ACTIVE_GPU_JOBS` | Start at `100` and `1`; raise only after cost and load evidence             |
...
to prove idempotency. Confirm the Vercel cron is
registered as `*/5 * * * *`, returns success, drains any reported backlog, and is alerting; do not infer this from `vercel.json`
alone.
...
journey, confirm the five-minute cleanup registration and alerts, and watch error/cost signals through
the first cold start. Stop or roll back on any release mismatch, model fallback, public endpoint,
missing scan/SBOM, migration drift, retention failure, secret exposure, physical-device failure,
or unexplained error-rate increase.
